 ...
 *  flex, où inverser l'ordre déplace les éléments). Les pourcentages se
 *  résolvent contre le parent immédiat (.content, toujours dimensionné en
 *  CARD_WIDTH_PT x CARD_HEIGHT_PT) — ce composant peut donc être imbriqué
 *  aussi bien dans une <Page> à la taille de la carte (génération à l'unité)
 *  que dans un <View> de même taille positionné sur une planche A4 (lot). */
...
const styles = StyleSheet.create({
  ...
  // empiriquement, cf. historique. Règle valable aussi sur la planche en
  // lot : chaque carte reste imbriquée dans un unique conteneur wrapper.
  watermark: {
    position: "absolute",
...
export interface CarteScolaireFaceProps {
  content: CardContent;
}

/** Le recto complet d'une carte scolaire — logique visuelle partagée entre
 *  la génération individuelle (CarteScolairePDF) et la planche en lot
 *  (PlancheCartesScolairesPDF), pour ne jamais dupliquer ce gabarit. */
export default function CarteScolaireFace({ content }: CarteScolaireFaceProps) {
  return (
