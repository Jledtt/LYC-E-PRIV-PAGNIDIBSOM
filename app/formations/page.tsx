...
export default function FormationsPage() {
  return (
    <>
      ...
      <section id="lycee" className="max-w-5xl mx-auto px-4 py-14 sm:py-20 scroll-mt-20">
        ...
        <div className="grid sm:grid-cols-2 gap-5 mb-8">
          {c.lycee.series.map((serie) => (
            <article key={serie.code} className="p-5 border-2 border-accent-200 rounded-xl">
              <span className="inline-block bg-accent-500 text-neutral-900 font-bold text-xs px-2.5 py-1 rounded mb-3">
                Série {serie.code}
              </span>
          ...
          <div className="flex-1">
            ...
            <ul className="flex flex-col gap-2">
              {c.lycee.atouts.map((a) => (
                <li key={a} className="flex items-start gap-2 text-sm text-neutral-600">
                  <span className="text-accent-600 mt-0.5 shrink-0" aria-hidden="true">
                    ✓
                  </span>
                  {a}
                </li>
      ...
      <section id="technique" className="max-w-5xl mx-auto px-4 py-14 sm:py-20 scroll-mt-20">
        <div className="mb-8">
          <span className="inline-block bg-neutral-100 text-neutral-700 text-xs font-semibold px-3 py-1 rounded-full uppercase tracking-wide mb-4">
            {c.technique.classes}
          </span>
          <h2
            className="text-3xl font-bold text-primary-800 mb-4 heading-serif"
          ...
          <p className="text-neutral-600 leading-relaxed max-w-2xl">{c.technique.description}</p>
        </div>

        {/* Filières */}
        <div className="grid sm:grid-cols-2 gap-6 mb-8">
          {c.technique.filieres.map((filiere) => (
            <article
              key={filiere.code}
              className="p-6 border-2 border-neutral-200 rounded-xl bg-white"
            >
              <span className="inline-block bg-primary-800 text-white text-xs font-bold px-2.5 py-1 rounded mb-3">
                {filiere.code}
              </span>
        ...
        {/* Image salle technique + atouts */}
        <div className="grid sm:grid-cols-2 gap-6">
          <div className="relative h-52 rounded-xl overflow-hidden">
            <Image
              src="/images/salle-2.jpg"
              alt="Salle de cours technique au Lycée Privé Pagnidibsom"
              fill
              className="object-cover"
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-4">
            {c.horaires.items.map((item) => (
              <div
                key={item.label}
                className="flex-1 bg-white border border-neutral-200 rounded-xl p-4 text-center"
              >
