...
import { rejectPaiementAction } from "./actions";

interface RejeterPaiementFormProps {
  paiementId: string;
}

export default function RejeterPaiementForm({ paiementId }: RejeterPaiementFormProps) {
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const result = await rejectPaiementAction(paiementId, motif);
      if (!result.success) {
  ...
  return (
    <div className="flex flex-col gap-2 w-full max-w-md">
      <label htmlFor={`motif-${paiementId}`} className="text-sm font-medium text-neutral-800">
        Motif du rejet (visible par le parent)
      </label>
      <textarea
        id={`motif-${paiementId}`}
        value={motif}
